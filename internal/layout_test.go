...
func TestResponsiveBreakpoints(t *testing.T) {
	tests := []struct {
		...
		{150, "wide"},   // >120 chars → 3 columns
		{100, "medium"}, // 80-120 chars → 2 columns
...
func TestModelLayoutAdaptation(t *testing.T) {
	...
	tests := []struct {
		...
		{"Edge case - exactly 120", 120, 40, 2},
...
func TestNavigationLogic(t *testing.T) {
	...
	model = newModel.(ui.Model)
	if model.GetActiveColumn() != 1 {
		t.Errorf("Expected active column 1, got %d", model.GetActiveColumn())
	}
	...
	if model.GetActiveColumn() != 0 {
		t.Errorf("Expected active column 0, got %d", model.GetActiveColumn())
	}

	// Test boundary conditions - can't go left from column 0
	newModel, _ = model.Update(tea.KeyMsg{Type: tea.KeyLeft})
	model = newModel.(ui.Model)
	if model.GetActiveColumn() != 0 {
		t.Errorf("Should stay at column 0, got %d", model.GetActiveColumn())
	}
	...
	if model.GetActiveColumn() != 2 {
		t.Errorf("Expected active column 2, got %d", model.GetActiveColumn())
	}

	// Test boundary conditions - can't go right from last column
	newModel, _ = model.Update(tea.KeyMsg{Type: tea.KeyRight})
	model = newModel.(ui.Model)
	if model.GetActiveColumn() != 2 {
		t.Errorf("Should stay at column 2, got %d", model.GetActiveColumn())
	}
}
...
func TestStateTransitions(t *testing.T) {
	...
	model = newModel.(ui.Model)
	if model.GetState() != ui.SearchMode {
	...
	model = newModel.(ui.Model)
	if model.GetState() != ui.MainNavigation {
