...
// Helper function for large dataset generation
func generateLargeMCPDataset(count int) []types.MCPItem {
	mcps := make([]types.MCPItem, count)
	mcpTypes := []string{"CMD", "SSE", "JSON", "HTTP"}
	
	for i := 0; i < count; i++ {
		mcps[i] = types.MCPItem{
			Name:    fmt.Sprintf("perf-mcp-%04d", i),
			Type:    mcpTypes[i%len(mcpTypes)],
			Active:  i%3 == 0, // Every third MCP is active
			Command: fmt.Sprintf("performance-command-%04d", i),
		}
	}
